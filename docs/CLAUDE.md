...
- **Admin metrics P2 (deferred from P34, ADR-0017)**: (a) before any admin dashboard UI ships, switch `/api/admin/metrics/*` from the shared `CRAVINGS_ADMIN_TOKEN` gate to the per-user `is_admin` gate (`Depends(_get_user)` + `is_admin_email`) — a browser can't safely hold the shared token; small localized swap on 4 routes. (b) Add event instrumentation (`app_open` / `session_start` / `screen_view` / onboarding funnel) so retention means more than "swiped" — current DAU/WAU/MAU + cohorts are a lower bound. (c) Optional admin dashboard screen reusing FlavorRadar/AxisBars.
