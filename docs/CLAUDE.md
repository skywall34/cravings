...
| P36 | Catalog completeness backfill (Jul 2026, ADR-0020): 79 tagged items with no `image_slug` (long tail Wikimedia/Openverse never covered) — user AI-generated PNGs, ingested via new `scripts/ingest_missing_images.py` (reuses `_slugify`/`_process_and_save`/`update_food_item_image`, bypasses the CC/PD license allow-list + VLM judge; `image_author="AI generated"`, `image_review_status="approved"`). Also backfilled 493 tagged items (ids 511–1003, the world-cuisine cohort) that were never embedded — `embed_items.py` simply hadn't been re-run since the catalog grew. New invariant test `tests/test_catalog_completeness.py` asserts every tagged item has both `image_slug` and `embedding` (skips if real DB absent, like `test_recommend_real_data.py`). **Also fixed a GDPR-erasure bug found by the post-deploy smoke test**: `db.delete_user()` only deleted the `users` row — `consumed_snapshot_items` (written on every swipe) and `billing_sessions` both FK-reference `users(id)` with no cascade, so any account that had ever swiped or checked out 500'd on `DELETE /api/users/me`. Fixed in `db/users.py:delete_user()`; regression test `tests/test_api.py::test_delete_account_after_swipe`. 445 tests pass. | ✅ Complete |
...
**P36 catalog completeness backfill shipped and deployed (2026-07-22).** 79 items imaged (AI-generated, ADR-0020), 493 items embedded (world-cuisine cohort, ids 511–1003). Local invariant verified: 0 tagged items missing `image_slug`/`embedding`; 445 tests pass; typecheck/build clean. Deployed via `docker_build.sh` (new DB seed baked, Watchtower redeployed) + `rsync images/food/`; prod spot-check confirmed (200 + immutable cache on a plain-ascii slug and an accented slug).
...
# Ingest AI-generated images for items still missing one (ADR-0020) — bypasses
# the CC/PD license allow-list + VLM judge. Put {stem}.png in images/missing/.
...
Claude Code must NEVER run git commands that change repo state — no `git add`, `git commit`, `git push`, `git merge`, `git rebase`, `git reset`, etc. Instead: print exact commands for user to review and run themself. Read-only git commands (`status`, `diff`, `log`, `show`) OK to run directly.
