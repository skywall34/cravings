...
- **Platform pivot**: React Native dropped (WSL2 dev environment, no iOS deploy). React + Vite 5 web app instead. Vite 5 required — Vite 9 incompatible with Node 20.18.x on WSL2. *(Update P19, June 2026: Android initially shipped as a Capacitor wrap — no UI rewrite. Update P32, June 2026: PWA is now the primary mobile channel for both iOS and Android — installable via browser, no App Store required. Capacitor `android/` kept as legacy. See ADR-0016 and `docs/ANDROID_HANDOFF.md`.)*
